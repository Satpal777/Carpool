app.controller('myridesCtrl', function ($scope, $http, $rootScope, $location) {
    ...
    $scope.refresh = () => {
        var myridepublished = [];
        $http.get(`${$rootScope.myurl}/api/get-ridesByuid?uid=${localStorage.id}`).then((res) => {
        ...
        var myreqsride = [];
        $http.get(`${$rootScope.myurl}/api/get-sharedridesBypid?pid=${localStorage.id}`).then((res) => {
            for(let i=0;i<res.data.rides.length;i++){
                $http.get(`${$rootScope.myurl}/api/get-ridesByid?_id=${res.data.rides[i].rid}`).then((response) => {
                    myreqsride.push(
                        {
                            srnum: i+1,
