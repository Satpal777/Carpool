app.controller('requestrideCtrl', function ($scope, $http, $routeParams,$rootScope, $location) {
    ...
    function calculateCost(slat, slon, dlat, dlon, map) {
        $http.get(`https://api.geoapify.com/v1/routing?waypoints=${slat},${slon}|${dlat},${dlon}&mode=drive&apiKey=a1e9172dce404ffb94a2db27e819a553`).then((res) => {
            L.Routing.control({
                waypoints: [
                    L.latLng(slat, slon),
                    L.latLng(dlat, dlon)
    ...
    const geocoding = async (myfun) => {
        ...
        await $http.get(`https://api.geoapify.com/v1/geocode/search?text=${urlfrom}&lang=en&limit=1&format=json&apiKey=a1e9172dce404ffb94a2db27e819a553`).then((res) => {
            $scope.source = {
        ...
        await $http.get(`https://api.geoapify.com/v1/geocode/search?text=${urlto}&lang=en&limit=1&format=json&apiKey=a1e9172dce404ffb94a2db27e819a553`).then((res) => {
            $scope.dest = {
                lat: res.data.results[0].lat,
                lon: res.data.results[0].lon
            }
    ...
    $http.get(`${$rootScope.myurl}/api/get-ridesById?_id=${rideId}`).then((res) => {
    ...
    $scope.submitride = () => {
        ...
        $http.post(`${$rootScope.myurl}/api/add-sharedride`, data).then((res) => {
            var avlseats = JSON.stringify({
                avlseats:$scope.avlseats-1
            })
            $http.put(`${$rootScope.myurl}/api/update-ride?_id=${$routeParams.id}`,avlseats).then((response) => {
                $location.path('')
            })
        })
    }
